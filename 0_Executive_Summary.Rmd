...
complex (GBYR, \emph{`r spp.sci`}) resource as in U.S. waters off the coast of 
...
and are rare north of Point Arena ($38^\circ 57^\prime$ N. latitude). The range 
...
the years of World War II, ranging between `r min(round(Tot.catch.df[Tot.catch.df[,1] >= 1939 & Tot.catch.df[,1] <= 1945,2],0))` to `r max(round(Tot.catch.df[Tot.catch.df[,1] >= 1939 & Tot.catch.df[,1] <= 1945,2],0))` metric  tons (mt) per year (Figure \ref{fig:Exec_catch2}). Commercial landings 
...
commercial discards was available starting in 2004, and were estimated prior to then. The catches 
...
table was based around the emphasis given to a fishery-dependent index of abundance for the 
recreational fleet. The stock was found to be at 97% depletion. 
...
This assessment covers the area from Cape Mendocino to the U.S./Mexico border 
...
fleet north of Point Conception.
...
updated maturity curve, a number of new indices, and new conditional length at age data.  
...
to 1978, when the recruitment deviations were first estimated (Figure \ref{fig:Spawnbio_all} 
and Table \ref{tab:SpawningDeplete_mod1}). The stock declined from 1978 to 1994, followed by a period 
increase from 1995 to 2003. From 2004-2018 the stock has been in decline, with an increase in total biomass 
since 2016 and stable spawning output since 2018. The 2018 estimated spawning output relative to unfished 
...
in the estimated spawning output is high, (95% asymptotic interval: 337-767 million eggs).
\FloatBarrier
...
Estimated recruitment pulses were estimated for a number of other years including 1994-1995 
and 2014-2015.
...
with no fishing. The SPR over the last decade increased ranged from 0.64 to 0.77 from 2009-2015, 
and ranged from 0.80 to 0.82 from 2016-2018.
...
0.439 (95% asymptotic interval: 0.341-0.536, corresponding 
...
Total mortality estimates from the NWFSC are not yet available are not yet available 
...
structure of complex and contribution of each of the two species to the complex 
and biological parameters differences.  Additionally, there is currently no 
...
the PFMC adopted ACL of 114 mt.
...
estimates of growth were different than the internal estimates.  Given that 
...
of uncertainty. The low state of nature fixed $k$ at 0.46 and the L1 and L1 parameters 
are estimated.  The high state of nature fixes all growth parameters, $k$ = 0.248, 
L1 = 13.8, and L2 = 28.5.  The L1 and L2 parameters were fixed in the high state of 
nature as they weren't well estimated by the model in the high state of nature. 
The growth parameters in the base model were estimated as $k$ = 0.107, L1 = 13.4, 
and L2 = 28.8.

The forecasted buffer ramp was calculated assuming a category 2 stock, with  sigma = 1.0 
and a $p^*$ = 0.45. The buffer is assumed to be 1.0 for 2019 and 2020 since the catches 
are assumed, and ranges from 0.874 in 2021 ramping to 0.803 in 2030. Current medium-term forecasts 
based on the alternative states of nature project that the stock will remain above the target 
threshold of 40% for all but two scenarios (Table \ref{tab:Decision_table_mod1}).  The 
low state of nature with the high state of nature catches results in a stock at 
26.4% of unfished in 2030 and the base state model with the 
high state of nature catches results in a stock at 33.2% of unfished in 2030.  The base case 
model with the base case catches results in an increasing stock over the period from 2021-2030. 
...
\item Investigate structure of complex and contribution of each species to the complex and biological parameters differences.  Also, investigate regional differences within species. 

\item Structure of complex and contribution of each species to the complex and biological parameters differences.  Also regional differences within species.  
    \begin{enumerate}
     \item Life history studies
     \item Research to identify proportion of each species in population and in catch
    \end{enumerate}

\item Historical catch reconstruction data, more so for the recreational fishery

\item Refine PISCO survey and analysis to better identify age-0 in each month of survey, which 
would benefit from occasional sampling later in the year to identify size at age-0.

\item Refine CCFRP survey index and continue coastwide sampling.

\item Recommend commercial and recreational data length and age collection. 

\item Integrating data collection across the different research needs to improve efficacy of data collection.

\item Environmental drivers/co-variates for biological parameters, particularly recruitment.

\item Make recreational survey dockside (PR) data available at trip level because of differences in the selectivity. 

\item Resolve differences between CalCOM and PacFIN expanded length compositions. 
