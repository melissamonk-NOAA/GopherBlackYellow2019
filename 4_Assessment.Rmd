...
Attempts to fit asmyptotic and dome-shaped selectivity to the CCFRP data resulted in 
poor estimation, large standard deviations, or a lack of fit to the data. The aggregated 
CCFRP length composition over time was similiar to the length composition data of the 
...
smaller GBYR, which is reflected in the asmyptotic selectivity shifted to the left of all 
...
2013 and 2014.  The increase in relative abundance observed in 2018 was not fit by the model, even with the added variance. The variance added to this survey was the highest for all indices.
...
suggests an increase from 2010 to 2013 and another increase after a declin in 2016. The 
...
to the uppder bound of the input standard error.  The increases in age-0 GBYR in 2001, 
...
here by the fit. The increasing trends in abundance from 2016-2018 was also not captured by the model fit, and the fit suggests a declining trend over the entire time series from 2007-2018.
...
larger fish, which is likely due to the mirroring of the selectivitity.  However, 
on an annual basis, there is a tradeoff with the CCFRP with under-fitting of the larger 
...
log($R_0$) of 8.0 (Table \ref{tab:like_profiles}). The individual surveys tend to minimize 
at the upper bound or jut below, while the length composition data has conflicting trends, e.g., CCFRP and commercial fleets, or the upper bound, e.g., PISCO and 
the recreational north fleets (Figures \ref{fig:profile_R0_piner}-\ref{fig:profile_R0_piner2}). 
The data all consistently minimize around 8.5 and the recreational fleet south of Point Conception 
minimize $R_0$ at the lower bound.  (Figure \ref{fig:profile_R0_like}). Over the range of values of $R_0$, depletion ranged from 0.38-0.59 (Figure \ref{fig:profile_R0_depl}).

For steepness, the negative log-likelihood reaches a minimum around a steepness 
near the upper bound (Figure \ref{fig:profile_h_like} and 
Table \ref{tab:like_profiles} and Figure \ref{fig:profile_m_depl}). 
The length composition likelihood components declined towards the uppder bound for Deb's 
CPFV and the recreational south fleets, while the other fleets either 
reached a minimum around 0.55 or at the lower bound.  (Figure \ref{fig:profile_h_piner}). 
Overall changes in the survey likelihood across the range of steepnes was less than 2.0, 
...
The negative log-likelihood was minimized at a natural mortality value near 0.2, slightly 
...
recreational fleet north of Point Conception, and CCFRP data minimizing wars the lower 
...
minimizes at the upper bound. The PISCO and CCFRP minimized around a natural mortality value 
of 0.22, while the PISCO age-0 and overall survey likelihood minimized at the upper bound of 0.3 
(Figure \ref:{profile_m_piner2}). The relative depletion for GBYR ranged from 0.32-0.59 across 
