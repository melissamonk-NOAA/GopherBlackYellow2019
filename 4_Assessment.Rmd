...
This led to poor convergence during the iterative search for Fspr under certain 
...
executable on June 30. Apart from the iterative Fspr search mentioned above, 
...
onboard CPFV survey and recreational fleet were modeled using the double logistic. The 
...
Starting in 1999, the CDFW began the onboard observer program (described in the indices of 
...
the Dirichlet-Multinomial likelihood was applied to a number of of the fleets with composition 
...
_Growth._Five growth parameters were estimated for the one-sex model: three von 
...
was repeated 240 times and the minimum was reached in 97% of the runs  (Table \ref{tab:jitter}).
...
was 0.247, whereas the internal estimate was much lower at 0.121. Using the Schnute 
parameterization with the age for L1 set at 2 and L2 at 23, the external estimates of lengths 
at Amin and Amax were 13.80 and 28.22, respectively.  The internal estimates of the lengths 
for Amin and Amax were 9.72 and 28.36, respectively. Natural mortality was estimated 
in the base model and natural mortality and the growth parameter k are negatively 
correlated. The model sensitivity to growth and natural mortality are explored in
the sensitivities.  
...
The PISCO study only samples in 0-20 m depth and observers small young-of-the-year 
...
recruitment, and another high recruitment pulse in the late 2010s. The PISCO kelp forest 
fish survey is the only fishery-independent survey that observed young-of-the-year GBYR 
in this assessment.
...
true for one of the peaks at 25 cm for the PISCO data, that were the nosiest of all the length 
...
estimated at 0.16, and the other growth parameters both decrease.  
...
almost identical for the models using harmonic means and Francic weights, both 
...
minimize around 8.5 with a smal change in likelihood at larger values of $R_0$ 
(Figure \ref{fig:profile_R0_like}). Over the range of values of $R_0$, depletion ranged 
from 0.22-0.05 (Figure \ref{fig:profile_R0_depl}).
...
was minimized as the second to largest value of $M$ run, 026. (Table \ref{tab:like_profiles}). 
