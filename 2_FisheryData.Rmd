...
the vessel must have fished in appropriate habitat during the trip. 
...
were grouped into three regions, north of Sonoma county, Sonoma county through 
Santa Cruz county, and San Luis Obispo county.  Area_x is a measure of distance 
...
The index excluding the false positives resulted in unrealistic values for the index 
in 1999 (Figure \ref{fig:MRFSS_index_N_SM_falsepos}).  Because the catch included in this index is "sampler-examined" and the samplers are well trained in species identification, the index 
used in the assessment includes the trips identified as false positives by the 
Stephens-MacCall filter. There is not a lot of contrast in the index, excpect for 
a small increase in 1986.  The final index values and associated log standard error 
included in the assessment can be found in Table \ref{tab:Indices}.


*MRFSS filtering and index standardization for south of Pt. Conception*
Prior to the Stephens-MacCall filter, a total of 7,334 trips were retained for the analysis. 
As expected, positive indicators of GBYR trips included several nearshore speces, e.g., 
kelp rockfish, treefish, and black croaker, whiel the strongest counter-indicator
...
level os 0.22 (Table \ref{tab:Fleet11_contingency}). 

Catch per angler hour (CPUE; number of fish per angler hour) was modelled using 
the deltaGLM approach.  A negative binomial model was explored, but the proportion of 
false positives was not well estimated in the negative binomial models. This is likely 
due to the facts that MRFSS sampling effort was higher south of Pt. Conception, and GBYR 
are also rare south of Pt. Conception. Model selection using Akaike Information Criterion 
(AIC) supported inclusion of year, region, area_x, and 2-month waves. Counties 
were grouped into three regions,Santa Barbara to Ventura counties, Los Angeles and Orange counties, and San Diego county.  Area_x is a measure of distance from shore, a categorical variable indicating whether most of the fishing occurred inside or outside three nautical 
miles from shore. The full model was selected for both Stephens-MacCall filtering methods.

The resulting index represents different years than that for north of Pt. Conception 
(Table \ref{tab:Indices}). The index starts in 1980 with continuous data through 1986, 
and three additional years in 1996, 1998 and 1999.  The index increases through 1983 and a 
marked decrease to 1986.  The index in the 1990s is non-informative.
